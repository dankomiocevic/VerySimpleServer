...
// NewRootCommand enables all children commands to read flags from CLI flags, environment variables prefixed with VSS, or config.yaml (in that order).
func NewRootCommand() *cobra.Command {
	...
	configPaths := []string{"/etc/vss", "$HOME/.vss", "."}
	...
	return &cobra.Command{
		...
		Long: `VerySimpleServer is a server that performs simple tasks in a reliable and fast way.
